 ...
 * This is the SQL to run in Supabase SQL Editor to set up the database
 */
...
CREATE INDEX IF NOT EXISTS idx_wines_colour_style ON wines(colour_style);
...
DROP POLICY IF EXISTS "Everyone can read active wines" ON wines;
CREATE POLICY "Everyone can read active wines" ON wines
  FOR SELECT USING (status = 'Active');

DROP POLICY IF EXISTS "Authenticated users can read all wines" ON wines;
...
DROP POLICY IF EXISTS "Authenticated users can create menus" ON menus;
CREATE POLICY "Authenticated users can create menus" ON menus
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Authenticated users can read their own menus" ON menus;
CREATE POLICY "Authenticated users can read their own menus" ON menus
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Authenticated users can update their own menus" ON menus;
CREATE POLICY "Authenticated users can update their own menus" ON menus
  FOR UPDATE USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Authenticated users can delete their own menus" ON menus;
CREATE POLICY "Authenticated users can delete their own menus" ON menus
  FOR DELETE USING (auth.role() = 'authenticated');

-- Create RLS policies for menu_sections
DROP POLICY IF EXISTS "Authenticated users can manage menu_sections" ON menu_sections;
